...
local function GetRowFromPool(parent)
    ...
    if not row then
        ...
        -- Remove mount preview click handler (no DressUpFrame opening)
...
function RaidMount.UpdateRowContent(row, data)
    ...
    -- Column positions - matching headers exactly
    local columns = {
        {pos = 10, width = 30},   -- Icon
        {pos = 45, width = 170},  -- Mount Name (tighter)
        {pos = 220, width = 110}, -- Raid/Source (tighter)
        {pos = 335, width = 90},  -- Boss (tighter)
        {pos = 430, width = 60},  -- Drop Rate (tighter)
        {pos = 495, width = 80},  -- Expansion (tighter)
        {pos = 580, width = 90},  -- Lockout (new)
        {pos = 675, width = 50},  -- Attempts (tighter)
        {pos = 730, width = 40},  -- Status (tighter)
        {pos = 775, width = 90},  -- Last Attempt (tighter)
    }
    ...
    if not row.lockoutIcon then
        ...
        row.lockoutIcon:SetSize(16, 16)
    end
    row.lockoutIcon:SetPoint("LEFT", row, "LEFT", columns[7].pos + 10, 0)
    local lockoutStatus = (data.lockoutStatus or "Unknown"):lower()
    if lockoutStatus == "no lockout" then
        row.lockoutIcon:SetTexture("Interface\\RaidFrame\\ReadyCheck-Ready")
        row.lockoutIcon:SetVertexColor(0, 1, 0, 1)
        row.lockoutIcon:Show()
    elseif lockoutStatus == "unknown" then
        row.lockoutIcon:SetTexture("Interface\\RaidFrame\\ReadyCheck-Waiting")
        row.lockoutIcon:SetVertexColor(0.7, 0.7, 0.7, 1)
        row.lockoutIcon:Show()
    else
        row.lockoutIcon:SetTexture("Interface\\RaidFrame\\ReadyCheck-NotReady")
        row.lockoutIcon:SetVertexColor(1, 0, 0, 1)
        row.lockoutIcon:Show()
    end
    if row ~= RaidMount.headerRow then
        row.texts[6]:SetText("")
    end
    
    -- Attempts
    row.texts[7]:SetPoint("LEFT", row, "LEFT", columns[8].pos, 0)
    local attempts = data.attempts or 0
    local attemptsColor = attempts > 0 and "|cFFFFFF00" or "|cFFFFFFFF"
    row.texts[7]:SetText(attemptsColor .. attempts .. "|r")
    row.texts[7]:SetWidth(columns[8].width)
    row.texts[7]:SetJustifyH("CENTER")
    ...
    row.statusIcon:SetPoint("LEFT", row, "LEFT", columns[9].pos + 10, 0) -- Center the icon in the column
    ...
    -- Last Attempt
    row.texts[9]:SetPoint("LEFT", row, "LEFT", columns[10].pos, 0)
    row.texts[9]:SetText(data.lastAttempt or "Never")
    row.texts[9]:SetWidth(columns[10].width)
    row.texts[9]:SetJustifyH("CENTER")
    row.texts[9]:SetTextColor(0.7, 0.7, 0.7, 1) -- Gray
end
...
local function UpdateVisibleRows()
    ...
    if not RaidMount.headerRow then
        local headerRow = CreateFrame("Frame", nil, RaidMount.ScrollFrame.scrollChild)
        headerRow:SetHeight(rowHeight)
        headerRow:SetPoint("TOPLEFT", RaidMount.ScrollFrame.scrollChild, "TOPLEFT", 0, 0)
        headerRow:SetPoint("TOPRIGHT", RaidMount.ScrollFrame.scrollChild, "TOPRIGHT", 0, 0)
        headerRow.texts = {}
        local headerTitles = {"", "Mount", "Raid", "Boss", "Drop Rate", "Expansion", "Lockout", "Attempts", "Status", "Last Attempt"}
        local columns = {
            {pos = 10, width = 30},   -- Icon
            {pos = 45, width = 170},  -- Mount Name
            {pos = 220, width = 110}, -- Raid
            {pos = 335, width = 90},  -- Boss
            {pos = 430, width = 60},  -- Drop Rate
            {pos = 495, width = 80},  -- Expansion
            {pos = 580, width = 90},  -- Lockout
            {pos = 675, width = 50},  -- Attempts
            {pos = 730, width = 40},  -- Status
            {pos = 775, width = 90},  -- Last Attempt
        }
        for i = 1, 10 do
            local col = columns[i]
            local text = headerRow:CreateFontString(nil, "OVERLAY", "GameFontNormal")
            text:SetFont(cachedFontPath, 13, "OUTLINE")
            text:SetPoint("LEFT", headerRow, "LEFT", col.pos, 0)
            text:SetWidth(col.width)
            text:SetJustifyH("CENTER")
            text:SetText(headerTitles[i] or "")
            headerRow.texts[i] = text
        end
        RaidMount.headerRow = headerRow
    end
